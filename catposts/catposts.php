...
/*
  Copyright (C) 2015  Author  (email : rew@rixom.org)
...
function cp_shortcode( $atts ) {
  ...
  foreach ($categories as $key => $value) {
    $return_html  .= '<li><h3><a href="'.get_category_link( $value->term_id ).'">';
      $return_html .= $value->name;
    $return_html .= '</h3>';
    $return_html .= cp_get_posts( $value->term_id );
    $return_html .= '</li>';
  }
  $return_html .='</ul>';
  return $return_html;
